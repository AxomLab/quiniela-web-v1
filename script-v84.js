/* ============================================================
   ...
   Cambios v8.5:
   - Corrige zona segura de avatares en carrera.
   - Quita duplicidad visual entre puntos y aciertos en la carrera.
   - Simplifica marcadores, popovers, ranking móvil y resumen.
   - Mantiene la lógica estable de datos.
   ============================================================ */
...
function renderRaceAvatar(p) {
  ...
  const img = hasAvatar
    ? `<img src="avatars/${escapeAttr(p.avatar)}" alt="${escapeAttr(p.alias)}" loading="eager" decoding="async" fetchpriority="high"
         onload="markAvatarLoaded(this, '.race-avatar')"
         onerror="retryAvatarImage(this, '.race-avatar')" />`
    : '';
...
function renderMiniAvatar(p) {
  const hasAvatar = Boolean(p.avatar);
  const img = hasAvatar
    ? `<img src="avatars/${escapeAttr(p.avatar)}" alt="${escapeAttr(p.alias)}" loading="eager" decoding="async"
         onload="markAvatarLoaded(this, '.mini-avatar')"
         onerror="retryAvatarImage(this, '.mini-avatar')" />`
    : '';
...
function markAvatarLoaded(img, selector) {
  const holder = img.closest(selector);
  if (!holder) return;
  holder.classList.add('has-image');
  holder.classList.remove('no-image');
}

function retryAvatarImage(img, selector) {
  const holder = img.closest(selector);
  ...
  if (retries < 3) {
    ...
    setTimeout(() => {
      img.src = `${cleanSrc}?avatarRetry=${Date.now()}-${retries + 1}`;
    }, 250 * (retries + 1));
    return;
  }
