/* ============================================================
   ...
   Cambios v8.2:
   - Corrige porcentaje visual: aciertos / partidosJugados * 100.
   - Muestra porcentaje sin decimales.
   - Mejora frases dinámicas de "Datos al azar".
   - Mejora contenido de "Partido caliente".
   - Mejora etiquetas y texto de top/bottom para hacerlo más divertido.
   - Corrige conteo de Partido caliente para pronósticos L/E/V.
   - Fuerza actualización de avatares para evitar caché cuando se reemplazan imágenes.
   ============================================================ */
...
function renderRaceAvatar(p) {
  ...
  const img = hasAvatar
    ? `<img src="avatars/${escapeAttr(p.avatar)}?v=${ASSET_VERSION}" alt="${escapeAttr(p.alias)}" loading="lazy"
...
function renderMiniAvatar(p) {
  const hasAvatar = Boolean(p.avatar);
  const img = hasAvatar
    ? `<img src="avatars/${escapeAttr(p.avatar)}?v=${ASSET_VERSION}" alt="${escapeAttr(p.alias)}"
